...
function parseBenchmarkCSV(text) {
  ...
  return lines
    ...
    .map((line) => {
      ...
      if (!dateStr || isNaN(closeVal)) return null;
      return { date: dateStr.trim(), close: closeVal };
    })
    .filter(Boolean)
...
async function loadBenchmarkSeries(symbol) {
  const res = await fetch(
    `https://stooq.com/q/d/l/?s=${symbol}.us&i=d&t=${Date.now()}`
  );
...
async function loadBenchmarkSeriesWithFallback(symbol, fallbackPath) {
  try {
    const liveData = await loadBenchmarkSeries(symbol);
    ...
    const fetchBenchmarks = async () => {
      try {
        const [sp500, nasdaq] = await Promise.all([
          loadBenchmarkSeriesWithFallback("spy", "/benchmark_sp500.csv"),
          loadBenchmarkSeriesWithFallback("qqq", "/benchmark_nasdaq.csv"),
        ]);
      ...
      >
        ...
        >
          ...
          >
            ...
            <button
              onClick={() => view !== "overlay" && setShowSp500((prev) => !prev)}
              className={`nav-btn ${showSp500 ? "active" : ""}`}
              style={{
                opacity: view === "overlay" ? 0.3 : 1,
                cursor: view === "overlay" ? "default" : "pointer",
                borderColor: showSp500 ? "#f59e0b" : undefined,
              }}
            >
              S&P
              <div
            ...
            <button
              onClick={() => view !== "overlay" && setShowNasdaq((prev) => !prev)}
              className={`nav-btn ${showNasdaq ? "active" : ""}`}
              style={{
                opacity: view === "overlay" ? 0.3 : 1,
                cursor: view === "overlay" ? "default" : "pointer",
                borderColor: showNasdaq ? "#22d3ee" : undefined,
              }}
            >
              Nasdaq
              <div
